...
同时，本项目还承担“自定义 Provider 接口验证与诊断”的职责：当平台接口、字段或赔率数据异常时，系统和配套文档应帮助开发者快速定位问题、对照官方文档并复测。
...
3. 多数据源整合
   - 支持 The Odds API。
   - 支持 `purebet`、`betdex`、`bookmaker_xyz`、`sx_bet`、`polymarket`。
   - `bookmaker_xyz` 必须覆盖项目固定 sport keys，以及 Azuro 当前公共 feed 可用的动态联赛 key：`azuro__<sport_slug>__<league_slug>__<country_slug>`。
4. 结果留存与诊断
   - 扫描结果可写入历史记录。
   - 可选保存最新扫描请求/结果。
   - 可选写入请求日志、Provider 快照、跨 Provider 匹配报告。
   - 需要支持对自定义 Provider 的端点可用性、响应结构与扫描结果合理性进行核验。
5. 前端交互
...
- 当 Provider 返回异常数据时，必须保留足够的诊断信息，支持开发者对照官方文档进行复核。
...
7. 自定义 Provider 出现接口异常时，文档中必须能定位到对应官方来源、测试方式和人工复查要求。
